...
export function NewsletterBand({ locale = "en" }) {
  const copy = locale === "es" ? {
    title: "Inspiración seleccionada, el Brasil más auténtico y viajes que van más allá de lo habitual.",
    thanks: "Gracias, ya formas parte de la lista.",
    join: "Quiero recibirla",
  } : {
    title: "Curated inspiration, authentic Brazil, and journeys designed beyond the ordinary.",
  ...
  return (
    <section className="bg-terra text-cream-50 py-12">
      <Container className="grid md:grid-cols-2 gap-8 items-center">
        ...
        >
          {done ? (
            <div className="bg-cream-50/20 rounded-full px-6 py-3.5">{copy.thanks}</div>
          ) : (
            <>
