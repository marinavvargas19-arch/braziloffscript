...
export default function BlogList({ locale = "en" }) {
  const { BLOG_POSTS, IMG } = locale === "es" ? ES : EN;
  const es = locale === "es";
    ...
    <>
      <section className="bg-paper border-b border-line">
        <Container className="py-12">
          <div className="text-[11px] tracking-[.22em] uppercase font-semibold text-terra mb-3">{es ? "Historias" : "Stories"}</div>
          <h1 className="font-serif text-[clamp(34px,4.5vw,56px)] leading-tight text-ink max-w-3xl">{es ? "Apuntes de viaje desde Brasil." : "Travel notes from inside Brazil."}</h1>
          <p className="mt-4 text-ink-soft text-[17px] max-w-2xl leading-relaxed">{es ? "Nuestras expertas escriben de vez en cuando sobre una región que las fascina, una comida que no pueden olvidar o cómo creen que debería diseñarse un viaje." : <>Our experts write occasionally — about a region they&apos;re obsessed with, a meal they can&apos;t stop thinking about, or a quiet rant about how trips should be designed.</>}</p>
        </Container>
      </section>

      <section className="py-12">
        <Container>
          <Link href={href(`/blog/${featured.slug}`)} className="group grid md:grid-cols-12 gap-8 items-center">
            ...
            <div className="md:col-span-5">
              <Badge variant="terra">{es ? "Destacado" : "Featured"}</Badge>
              ...
              <div className="mt-5 flex items-center gap-3">
                ...
                <div className="text-[13px] text-ink-soft">{es ? "Por" : "By"} <strong>{featured.author}</strong></div>
              </div>
            </div>
          </Link>
        </Container>
      ...
      <section className="pb-20">
        <Container>
          <div className="flex flex-wrap gap-2 mb-8">
            {cats.map(c=>(
              <button key={c} onClick={()=>setCat(c)} className={`px-4 py-2 rounded-full text-[13px] font-semibold border transition capitalize ${cat===c?"bg-leaf border-leaf text-cream-50":"border-line text-ink-soft hover:border-leaf"}`}>{c==="all"?(es ? "Todas las historias" : "All stories"):c}</button>
            ))}
          </div>
          <div className="grid md:grid-cols-3 gap-7">
            {filtered.map(p=>(
              <Link key={p.slug} href={href(`/blog/${p.slug}`)} className="group">
